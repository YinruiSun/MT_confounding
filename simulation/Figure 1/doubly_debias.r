...
for (k in 1:length(p_seq))
{
    ...
    result = foreach(i = 1:nrep, .combine = "rbind") %dopar% {
        ...
        result1 = test_stat_doubly(Y = data$Y, X = data$X, centralize = FALSE, rho = rho, rho_j = rho_j)
        data$X = NULL
        data$Y = NULL
...
savefile = paste("doubly_debias_type_", type, ".RData", sep = "")
save(n, p_seq, s, q, sigma, type, nu, mu, rho, rho_j, alpha, nrep, results, fdp, power, fdrs, powers, file = savefile)